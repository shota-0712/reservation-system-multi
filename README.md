...
ローカル開発ではPostgreSQLを使います。テーブル作成は後続Issueで追加するmigrationに任せ、ここではDBだけを用意します。
...
migration SQLは後続Issueで追加します。このIssueでは `db/migrations` は作りません。
...
後続IssueでSQLファイルが追加された後の実行コマンド例:
